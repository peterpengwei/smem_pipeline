...
static void mem_insert_seed_batched(const mem_opt_t *opt, int64_t l_pac, kbtree_t(chn) **tree, 
		smem_i **itr, int batch_size, const int *done, int bwt_batched_status,int start)
{
	int start_width;
	int batch_idx;
	int all_done = 0;
...
void mem_chain_batched(mem_chain_v *chn_batch, const mem_opt_t *opt, const bwt_t *bwt, int64_t l_pac, bseq1_t *seqs, int start, int batch_size, int batched_status)
{
	...
	mem_insert_seed_batched(opt, l_pac, tree, itr, batch_size, done, BWT_BATCHED_DO,start);
...
void bwt_batched_init(int batch_size)
{
	...
	mem_insert_seed_batched(NULL, 0, NULL, NULL, batch_size, 0, BWT_BATCHED_INIT,0);
...
void bwt_batched_free(int batch_size)
{
	...
	mem_insert_seed_batched(NULL, 0, NULL, NULL, batch_size, 0, BWT_BATCHED_FREE,0);
