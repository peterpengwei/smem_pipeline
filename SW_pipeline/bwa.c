...
extern unsigned int *SPL_BWT_ref;
extern unsigned int *SPL_CNT_table;
...
{
	printf("Entering Full Index reader\n");
	char *prefix;
	int l_hint;
	FILE *fp;
	l_hint = strlen(hint);
...
// HARP: Load ref. geno and CNT table in advance.
bwt_t *bwa_idx_load_bwt(const char *hint)
{
	printf("HARP: Load ref. geno and CNT table in advance.\n");
	char *tmp, *prefix;
	bwt_t *bwt;
	...
	printf("Entering bwt_resotre_bwt\n");
	bwt = bwt_restore_bwt(tmp);
	strcat(strcpy(tmp, prefix), ".sa");  // partial suffix array (SA)
	
	printf("Entering bwt_restore_sa\n");
	bwt_restore_sa(tmp, bwt);
	printf("Leaving bwt_restore_sa\n");
...
////////////////////////////////////////////////////////////////////////////////////
  // Geno Reference
 printf("start memcpy of Geno Reference\n");
  memcpy(SPL_BWT_ref,bwt->bwt,bwt->bwt_size<<2);
  //CNT table
 printf("finish memcpy of Geno Reference\n");
 printf("Licheng disabled CNT table\n");
  //memcpy(SPL_CNT_table,bwt->cnt_table,256 * sizeof(int));
  //printf("SPL_CNT_table in position\n");
//	*handshake = 2;
 // int watch_dog = *handshake;
//	double start_time = realtime();
//	fprintf(test_RW,"CNT_Start\n");
//	fprintf(stderr, "handshake = %d", *handshake);
//
//	while (watch_dog == 2) {
  //       watch_dog = *handshake;
    //     usleep(10);
  //}
//	fprintf(stderr, "handshake = %d", *handshake);
 // printf("CNT_table Done\n");
//	fprintf(test_RW,"CNT_Done = %fs\n", realtime() - start_time);
#else
	fprintf(stderr, "Bypass reference data loading\n");
#endif
  fclose(test_RW);
  *handshake = 0;
////////////////////////////////////////////////////////////////////////////////////
...
void bwa_print_sam_hdr(const bntseq_t *bns, const char *rg_line)
{
	int i;
	extern char *bwa_pg;
	err_printf("Licheng has shutdown SAM header output\n");
